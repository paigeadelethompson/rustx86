...
impl SerialController {
    pub fn new() -> Self {
        ...
        ports.push(Some(SerialPort::new(0x3F8)));
    ...
    pub fn read_byte(&mut self, port: u16) -> u8 {
        for p in self.ports.iter_mut().flatten() {
            if p.base_port == port {
                return p.read_byte(port as u8 & 0x07);
            }
        }
        0xFF
    }
    ...
    pub fn write_byte(&mut self, port: u16, value: u8) {
        for p in self.ports.iter_mut().flatten() {
            if p.base_port == port {
                p.write_byte(port as u8 & 0x07, value);
                break;
            }
        }
    }
...
impl SerialPort {
    ...
    pub fn get_status(&self) -> PortStatus {
        PortStatus {
            rx_ready: !self.rx_fifo.is_empty(),
            tx_ready: self.tx_fifo.len() < FIFO_SIZE,
            hardware_flow_enabled: self.hardware_flow_enabled,
            xon_xoff_enabled: self.xon_xoff_enabled,
            xon_state: self.xon_state,
        }
    }

    pub fn read_byte(&mut self, offset: u8) -> u8 {
        match offset {
            RBR => self.rx_fifo.pop_front().unwrap_or(0),
            IER => self.ier,
            LCR => self.lcr,
            MCR => self.mcr,
            LSR => self.lsr,
            MSR => self.msr,
            DLL if (self.lcr & 0x80) != 0 => self.dll,
            DLM if (self.lcr & 0x80) != 0 => self.dlm,
            _ => 0,
        }
    }
    ...
    pub fn write_byte(&mut self, offset: u8, value: u8) {
        match offset {
            ...
            LCR => self.lcr = value,
            MCR => self.mcr = value,
            DLL if (self.lcr & 0x80) != 0 => self.dll = value,
            DLM if (self.lcr & 0x80) != 0 => self.dlm = value,
            _ => (),
        }
    }
