...
impl Cpu {
    ...
    pub(crate) fn jmp_far(&mut self) -> Result<(), String> {
        ...
        self.regs.ip = offset;
        println!("JMP_FAR: old_cs:ip=0x{:04X}:0x{:04X}, new_cs:ip=0x{:04X}:0x{:04X}",
    ...
    pub(crate) fn call_near(&mut self) -> Result<(), String> {
        ...
        println!("CALL_NEAR: old_ip=0x{:04X}, offset=0x{:04X}, return_ip=0x{:04X}, new_ip=0x{:04X}",
                 old_ip, offset, old_ip.wrapping_add(2), self.regs.ip);
    ...
    pub fn ret_far(&mut self) -> Result<(), String> {
        ...
        println!("RET_FAR: old_cs:ip=0x{:04X}:0x{:04X}, new_cs:ip=0x{:04X}:0x{:04X}",
                 old_cs, old_ip, cs, ip);
    ...
    pub fn jcxz(&mut self) -> Result<(), String> {
        ...
        println!("JCXZ: old_ip=0x{:04X}, cx=0x{:04X}, offset=0x{:02X}", 
    ...
    pub(crate) fn loop_cx(&mut self) -> Result<(), String> {
        ...
        println!("LOOP: old_ip=0x{:04X}, old_cx=0x{:04X}, new_cx=0x{:04X}, offset=0x{:02X}",
...
mod tests {
    ...
    fn test_call_near() {
        ...
        // Check new IP
        assert_eq!(cpu.regs.ip, 0x0102, "Expected new IP 0x0102, got 0x{:04X}", cpu.regs.ip); // 0x0000 + 2 + 0x0100
        
        // Check that SP was decremented
        assert_eq!(cpu.regs.sp, 0xFFFE, "Expected SP 0xFFFE, got 0x{:04X}", cpu.regs.sp); // 0x0000 - 2
    }
    ...
    fn test_call_far() {
        ...
        println!("Reading return CS:IP from stack at physical addresses 0x{:05X} and 0x{:05X}", 
        ...
        assert_eq!(cpu.regs.sp, 0x1FFC, "Stack pointer not decremented correctly");
    }
    ...
    fn test_ret_far() {
        ...
        let stack_cs_addr = (cpu.regs.ss as u32) << 4 | 0x1FFE;
        let stack_ip_addr = (cpu.regs.ss as u32) << 4 | 0x1FFC;
        println!("Writing return CS:IP to stack at physical addresses 0x{:05X} and 0x{:05X}",
                stack_cs_addr, stack_ip_addr);
        ...
        assert_eq!(cpu.regs.sp, 0x2000, "Stack pointer not incremented correctly");
    }
    ...
    fn test_jcxz() {
        ...
        let code_addr = (cpu.regs.cs as u32) << 4 | (cpu.regs.ip as u32);
        println!("Writing jump offset at physical address 0x{:05X}", code_addr);
        ...
        assert!(cpu.jcxz().is_ok());
        assert_eq!(cpu.regs.ip, 0x1001, "IP incorrect after no jump when CX != 0"); // IP + 1 (no jump)
    }
    ...
    fn test_loop_cx() {
        ...
        // Calculate physical address for instruction
        let code_addr = (cpu.regs.cs as u32) << 4 | (cpu.regs.ip as u32);
        println!("Writing loop offset at physical address 0x{:05X}", code_addr);
