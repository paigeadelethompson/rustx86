...
impl CPU {
    pub(crate) fn execute_group1_rm8_imm8(&mut self, _group: u8) -> Result<(), String> {
        ...
        let result = match (modrm >> 3) & 0x07 {
            ...
            1 => rm_val | imm,             // OR
            2 => rm_val.wrapping_add(if self.regs.flags.get_carry() { 1 } else { 0 }).wrapping_add(imm), // ADC
            3 => rm_val.wrapping_sub(if self.regs.flags.get_carry() { 1 } else { 0 }).wrapping_sub(imm), // SBB
            4 => rm_val & imm,             // AND
        ...
        if (modrm >> 3) & 0x07 != 7 { // Don't write result for CMP
    ...
    pub(crate) fn handle_f6_group(&mut self) -> Result<(), String> {
        ...
        match (modrm >> 3) & 0x07 {
            ...
            5 => { // IMUL
                ...
                self.regs.flags.set_carry(result > 0x7F || result < -0x80);
                self.regs.flags.set_overflow(result > 0x7F || result < -0x80);
            }
            6 => { // DIV
                let rm_val = self.get_rm8(modrm)?;
                if rm_val == 0 {
    ...
    pub(crate) fn handle_f7_group(&mut self) -> Result<(), String> {
        ...
        match (modrm >> 3) & 0x07 {
            ...
            5 => { // IMUL
                ...
                self.regs.ax = result as u16;
                self.regs.dx = (result >> 16) as u16;
                self.regs.flags.set_carry(result > 0x7FFF || result < -0x8000);
                self.regs.flags.set_overflow(result > 0x7FFF || result < -0x8000);
            }
            6 => { // DIV
                let rm_val = self.get_rm16(modrm)?;
                if rm_val == 0 {
    ...
    pub(crate) fn handle_fe_group(&mut self) -> Result<(), String> {
        ...
        self.write_rm8(modrm, result)?;
        self.update_flags_arithmetic(rm_val, if (modrm >> 3) & 0x07 == 0 { 1 } else { !0 }, result, (modrm >> 3) & 0x07 == 1);
        Ok(())
    }
    ...
    pub(crate) fn handle_ff_group(&mut self) -> Result<(), String> {
        ...
        match (modrm >> 3) & 0x07 {
            0 | 1 => { // INC/DEC
                ...
                self.write_rm16(modrm, result)?;
                self.update_flags_arithmetic_16(rm_val, if (modrm >> 3) & 0x07 == 0 { 1 } else { !0 }, result, (modrm >> 3) & 0x07 == 1);
            }
            2 => { // CALL near
